...
namespace dynamixel_rdk_ros2
{
    ...
    bool MotorStatus::TxRx(uint8_t id, const std::pair<int, int> &control_table_address, T &value, const std::string &status_name, int mode)
    {
        ...
        if (mode == READ)
        {
            ...
            {
                RCLCPP_ERROR(logger_, "Unsupported data size for %s", status_name.c_str());
                return false;
            }

            if (dxl_comm_result_ != COMM_SUCCESS)
            {
                RCLCPP_ERROR(logger_, "Failed to get %s for ID %d: %s", status_name.c_str(), id, packet_handler_->getTxRxResult(dxl_comm_result_));
                return false;
            }

            if (dxl_error_ != 0)
            {
                RCLCPP_ERROR(logger_, "Dynamixel error while reading %s for ID %d: %s", status_name.c_str(), id, packet_handler_->getRxPacketError(dxl_error_));
                return false;
            }

            RCLCPP_INFO(logger_, "ID %d %s: %d", id, status_name.c_str(), static_cast<int>(value));
            return true;
        }
